...
class TransientTokenDataV2Api(object):
    """
    ...
    def get_transaction_for_transient_token_jti(self, jti, **kwargs):
        """
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.get_transaction_for_transient_token_jti(jti, callback=callback_function)

        :param callback function: The callback function
            for asynchronous request. (optional)
        :param str jti: The jti within the Transient Token jwt returned by the Unified Checkout application  (required)
        :return: None
                 If the method is called asynchronously,
                 returns the request thread.
        """

        self.logger.info("CALL TO METHOD `get_transaction_for_transient_token_jti` STARTED")

        kwargs['_return_http_data_only'] = True
        if kwargs.get('callback'):
            return self.get_transaction_for_transient_token_jti_with_http_info(jti, **kwargs)
        else:
            (data) = self.get_transaction_for_transient_token_jti_with_http_info(jti, **kwargs)
            return data

    def get_transaction_for_transient_token_jti_with_http_info(self, jti, **kwargs):
        """
        Get Transient Token Data v2
        Retrieve data captured through Unified Checkout. This API retrieves the detailed information associated with a Transient Token by looking it up in TMS and using its ID (the jti claim from the /flex/v2/tokens JWT response). The response returns a decrypted version of the Transient Token; however, PCI-sensitive payment data (PAN) is never returned and is always masked.<br><br> Example jti value: 1D42LRF04LYTMO3I1G8JX6GO6S1PUFM2R4CQLU51267E0EOQ7X2169A99674E16E
        This method makes a synchronous HTTP request by default. To make an
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.get_transaction_for_transient_token_jti_with_http_info(jti, callback=callback_function)
        ...
        for key, val in iteritems(params['kwargs']):
            if key not in all_params:
                raise TypeError(
                    "Got an unexpected keyword argument '%s'"
                    " to method get_transaction_for_transient_token_jti" % key
                )
            params[key] = val
        del params['kwargs']
        # verify the required parameter 'jti' is set
        if ('jti' not in params) or (params['jti'] is None):
            self.logger.error("InvalidArgumentException : Missing the required parameter `jti` when calling `get_transaction_for_transient_token_jti`")
            raise ValueError("Missing the required parameter `jti` when calling `get_transaction_for_transient_token_jti`")
        ...
        inbound_mle_status = "false"
        if MLEUtility.check_is_mle_for_api(self.api_client.mconfig, inbound_mle_status, "get_transaction_for_transient_token_jti,get_transaction_for_transient_token_jti_with_http_info"):
                body_params = MLEUtility.encrypt_request_payload(self.api_client.mconfig, body_params)
        
        isResponseMLEforApi = MLEUtility.check_is_response_mle_for_api(self.api_client.mconfig, "get_transaction_for_transient_token_jti,get_transaction_for_transient_token_jti_with_http_info")

        # Authentication setting
        auth_settings = []

        return self.api_client.call_api(f'/flex/v2/payment-details/{jti}', 'GET',
                                        path_params,
                                        query_params,
                                        header_params,
                                        body=body_params,
