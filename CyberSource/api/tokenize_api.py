...
class TokenizeApi(object):
    ...
    def tokenize(self, post_tokenize_request, **kwargs):
        """
        Tokenize
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.tokenize(post_tokenize_request, callback=callback_function)

        :param callback function: The callback function
            for asynchronous request. (optional)
        :param PostTokenizeRequest post_tokenize_request: (required)
        ...
        if kwargs.get('callback'):
            return self.tokenize_with_http_info(post_tokenize_request, **kwargs)
        else:
            (data) = self.tokenize_with_http_info(post_tokenize_request, **kwargs)
            return data

    def tokenize_with_http_info(self, post_tokenize_request, **kwargs):
        """
        Tokenize
        |  |  |  |   | --- | --- | --- | |The **Tokenize API** endpoint facilitates the creation of various TMS tokens such as Customers, Payment Instruments, Shipping Addresses, and Instrument Identifiers in a single operation. The request includes a processingInformation object, which specifies **\"TOKEN_CREATE\"** and the types of tokens to be created. The **tokenInformation** section of the request includes detailed information relevant to each token type. This includes attributes for Customers, Payment Instruments, Shipping Addresses, Instrument Identifiers and Transient Token data. The payload is flexible, allowing for different combinations of tokens to be created in a single request.|&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;|The **API response** includes a responses array, which details the outcome of the tokenization process for each requested resource type, such as Customer, Payment Instrument, Shipping Address, and Instrument Identifier. Each entry in this array provides an HTTP status code such as **201/200 for successful creations**, and a unique identifier for the newly created token.<br>In cases where token creation encounters issues, the response includes a **non-2XX** status code and an errors array for the affected resource. Each error object in the array details the **error type and a descriptive message** providing insight into why a particular token creation was not attempted or failed. 
        This method makes a synchronous HTTP request by default. To make an
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.tokenize_with_http_info(post_tokenize_request, callback=callback_function)
        ...
        all_params = ['post_tokenize_request', 'profile_id']
        ...
        if ('post_tokenize_request' not in params) or (params['post_tokenize_request'] is None):
            self.logger.error("InvalidArgumentException : Missing the required parameter `post_tokenize_request` when calling `tokenize`")
            raise ValueError("Missing the required parameter `post_tokenize_request` when calling `tokenize`")
        ...
        body_params = None
        if 'post_tokenize_request' in params:
            body_params = params['post_tokenize_request']
        
            sdkTracker = SdkTracker()
            body_params = sdkTracker.insert_developer_id_tracker(body_params, 'post_tokenize_request', self.api_client.mconfig.run_environment, self.api_client.mconfig.defaultDeveloperId)
            body_params = process_body(body_params)

        inbound_mle_status = "mandatory"
        if MLEUtility.check_is_mle_for_api(self.api_client.mconfig, inbound_mle_status, "tokenize,tokenize_with_http_info"):
                body_params = MLEUtility.encrypt_request_payload(self.api_client.mconfig, body_params)
        
        isResponseMLEforApi = MLEUtility.check_is_response_mle_for_api(self.api_client.mconfig, "tokenize,tokenize_with_http_info")

        # Authentication setting
        auth_settings = []

        return self.api_client.call_api(f'/tms/v2/tokenize', 'POST',
                                        path_params,
                                        query_params,
                                        header_params,
                                        body=body_params,
